    	...
    	<div class="container">
    		<form id="loginForm" action="" method="post" class="form-signin">
    			...
    			<div class="login-form" id="loginform">	
	    			...
	    			<input type="text" class="form-control" id="login" autofocus="autofocus" placeholder="username" required="required" name="sqluser"/>
	    			
	    			<label for="password" class="string control-label"><fmt:message key="dw4.password" /></label>
	    			<input type="password" class="form-control" id="password" placeholder="password" required="required" name="sqlpwd" />
		    ...
		    $(document).ready(function() {
		    	...
		    	// nel caso sia stata richiesto un URL jsf si esegue il redirect alla home 
		    	// gli unici URL inalterati sono quelli PrettyFaces (.pf) 
				if (currentUrl.match(".jsf$")) { 
					var index = currentUrl.lastIndexOf("/");
					if (index != -1)
						currentUrl = currentUrl.substring(0, index+1) + "home.jsf"; 
				}
		    	
		    	$("#loginForm").attr("action", currentUrl);
		    });
		    </script>
    	</div>
