	...
	private void fillFields(List<Vocabolario> vocabolari) throws Exception {
		...
		if (vocabolari != null && vocabolari.size() > 0) {
			for (int i=0; i<vocabolari.size(); i++) {
				...
				// aggiunti anche i campi cognome delle deleghe nel pannello di ricerca delle deleghe in acl
				// FIXME è un pò harcodato questo sistema, prevedere un fillField più generico?
				if (propertyName.equals("persint_cognome") 
						|| propertyName.equals("cogn_nome_delegante")
						|| propertyName.equals("cogn_nome_delegato"))
